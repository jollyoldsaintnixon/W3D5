...
class MCS
  ...
  def method_missing(name, *args)
    num = args[0]
    info_name = ('get_' + name.to_s + '_info').to_sym
    info = @snack_box.send(info_name, num)

    tastiness_name = ('get_' + name.to_s + '_tastiness').to_sym
    tastiness = @snack_box.send(tastiness_name, num)
